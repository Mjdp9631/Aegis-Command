...
function render() {
  ...
  const categoryCards = lane === "mind" ? `<div class="mastery-category-grid">${mindTypes.map(type => `<button class="mastery-category ${type === activeType ? "active" : ""}" data-mastery-clean-type="${type}"><small>${type.toUpperCase()}</small><strong>${entries.filter(entry => entry.category === type).length}</strong><small>${type === "Book" ? "books and reading notes" : "entries captured"}</small></button>`).join("")}</div>` : `<div class="mastery-subtabs">${tabs(types, activeType, "type")}</div>`;
