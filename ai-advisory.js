...
function legacyBuildContext({ operations, occurrences = [], missions, trades, recovery, mastery, projects, contentItems = [], tradeReviews = [], progressEvents = [], phase, directives = [], roadmap = [], deepWork = [], challenges = [], directorReviews = [], trainingSessions = [], trainingSets = [], weightLogs = [], foodLogs = [], activityEvents = [], accounts = [], groups = [], withdrawals = [], advisoryHistory = [], campaign = null }, operatingDate = operatingDayKey()) {
  ...
  const monthPnl = closed.filter((trade) => {
    const date = dateKey(trade.traded_at || trade.created_at);
    return date >= monthStart && date <= operatingDate;
...
async function gather(operatingDate = operatingDayKey(), mode = "scan") {
  ...
  const [operations, missions, trades, recovery, mastery, projects, phase, directives, roadmap, deepWork, challenges, directorReviews, occurrences, trainingSessions, trainingSets, weightLogs, foodLogs, activityEvents, accounts, accountDeposits, groups, withdrawals, advisoryHistory, feedback, calibrationReviews, contentItems, tradeReviews, tradeReviewCorrections, progressEvents, campaign, scenarios, capabilitySkills, capabilityLogs, financialFoundation] = await Promise.all([
    ...
    optionalQuery(supabase.from("account_deposits").select("*").order("deposited_at", { ascending: false }).limit(100)),
    ...
    optionalQuery(supabase.from("trade_review_corrections").select("*").order("created_at", { ascending: false }).limit(100)),
