...
function buildContext({ operations, missions, trades, recovery, mastery, projects, phase, directives = [], roadmap = [], deepWork = [], challenges = [], directorReviews = [] }) {
  ...
  return {
    ...
    mastery: { total_entries: mastery.length, recent: mastery.slice(0, 8).map((entry) => ({ category: entry.category, title: entry.title, date: dateKey(entry.created_at) })), deep_work: { recent_minutes: deepWork.filter((item) => Date.now() - new Date(item.created_at || item.logged_on).getTime() < 7 * 86400000).reduce((sum, item) => sum + Number(item.duration_minutes || 0), 0), recent: deepWork.slice(0, 8).map((item) => ({ area: item.area, focus: item.focus, minutes: item.duration_minutes, output: item.output, date: dateKey(item.created_at || item.logged_on) })) }, transmissions: { active: challenges.filter((item) => !item.completed_at).slice(0, 5).map((item) => ({ lane: item.lane, title: item.title, type: item.challenge_type })), recent_completed: challenges.filter((item) => item.completed_at).slice(0, 5).map((item) => ({ lane: item.lane, title: item.title, date: dateKey(item.completed_at) })) }, director_review: directorReviews[0] ? { quarter: directorReviews[0].quarter_key, wins: directorReviews[0].wins, bottlenecks: directorReviews[0].bottlenecks, standards: directorReviews[0].standards, next_focus: directorReviews[0].next_focus } : null },
