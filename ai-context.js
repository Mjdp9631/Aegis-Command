...
(function attachContext(root, factory) {
  ...
  const operationIsOngoing = (row) => !complete(row) && String(row?.status || "").toLowerCase() === "ongoing" && !row?._occurrence?.completed;
  const ongoingStart = (row, fallback) => dateOnly(row?.started_on || row?._occurrence?.started_on || row?.scheduled_date || row?.operation_date) || fallback;
  const ongoingDays = (row, day) => {
    const start = new Date(`${ongoingStart(row, day)}T12:00:00Z`);
    const end = new Date(`${day}T12:00:00Z`);
    if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime()) || end < start) return 1;
    return Math.max(1, Math.floor((end - start) / 86400000) + 1);
  };
  const ongoingDisplay = (row, day) => {
    if (!operationIsOngoing(row)) return row;
    const days = ongoingDays(row, day);
    return { ...row, scheduled_date: day, operation_date: day, ongoing_since: ongoingStart(row, day), ongoing_days: days, needs_attention: days >= LONG_RUNNING_OPERATION_DAYS };
  };
  const completedDisplay = (row) => {
    const completedOn = dateOnly(row?.completed_on || row?._occurrence?.completed_on);
    return complete(row) && completedOn ? { ...row, scheduled_date: completedOn, operation_date: completedOn } : row;
  };
  ...
  function buildDerivedInsights({ effectiveOperations, liveTrades, recovery, mastery, deepWork, contentItems, activityEvents, missions, directives, feedback, calibration, tradeReviews, scenarios, operatingDate }) {
    ...
    const ongoingRows = effectiveOperations.filter((row) => operationIsOngoing(row));
    const ongoingAttention = ongoingRows.filter((row) => ongoingDays(row, operatingDate) >= LONG_RUNNING_OPERATION_DAYS).map((row) => ({
      title: row.title,
      category: row.category,
      ongoing_since: ongoingStart(row, operatingDate),
      ongoing_days: ongoingDays(row, operatingDate),
      rollover_count: Number(row.rollover_count || row._occurrence?.rollover_count || 0),
      evidence_ids: [evidenceId(row._occurrence?.id ? "operation_occurrences" : "operations", idOf(row))].filter(Boolean),
    }));
    ...
    const windows = [7, 30, 90].reduce((result, days) => {
      ...
      const scopedLearning = [...mastery, ...deepWork, ...contentItems].filter((row) => within(rowDate(row, "logged_on", "created_at"), start, operatingDate));
    ...
    const days = [...new Set([...effectiveOperations.map(operationDay), ...closedTrades.map((row) => rowDate(row, "traded_at", "created_at")), ...recovery.map((row) => rowDate(row, "logged_on", "created_at")), ...mastery.map((row) => rowDate(row, "logged_on", "created_at")), ...deepWork.map((row) => rowDate(row, "logged_on", "created_at")), ...contentItems.map((row) => rowDate(row, "logged_on", "created_at"))].filter((day) => within(day, windowStart(operatingDate, 90), operatingDate)))].sort();
    const dayRecords = days.map((day) => {
      ...
      const dayLearning = [...mastery, ...deepWork, ...contentItems].filter((row) => rowDate(row, "logged_on", "created_at") === day);
  ...
  function buildContext(input = {}, operatingDate, mode = "scan") {
    ...
    const capabilities = records.capabilities || records.capabilitySkills || [];
    ...
    const accountDeposits = records.accountDeposits || records.deposits || [];
    ...
    mastery.forEach((item) => addEvidence("mastery_entries", item, item.logged_on || item.created_at, item.title || "Mastery entry logged", { category: item.category }));
    ...
    deepWork.forEach((item) => addEvidence("deep_work_logs", item, item.logged_on || item.created_at, "Deep work block logged"));
    ...
    contentItems.forEach((item) => addEvidence("content_items", item, item.logged_on || item.created_at, `${item.title || "Content"} record`));
    capabilities.forEach((item) => addEvidence("capability_skills", item, item.last_practiced_on || item.created_at, `${item.title || "Capability"} skill track`, { skill_type: item.skill_type, status: item.status, practice_count: item.practice_count }));
    capabilityLogs.forEach((item) => addEvidence("capability_skill_logs", item, item.practiced_on || item.created_at, "Capability practice logged", { pressure_level: item.pressure_level, result: item.result }));
    ...
    return {
      ...
      mastery: { total_entries: mastery.length, recent: mastery.slice(0, 10).map((entry) => ({ evidence_id: evidenceId("mastery_entries", idOf(entry)), category: entry.category, title: entry.title, date: dateOnly(entry.logged_on || entry.created_at) })) },
      activity_ledger: { total_events: activityEvents.length, by_metric: activityByMetric, recent: activityEvents.slice(0, 40).map((event) => ({ evidence_id: evidenceId(event.source_type || "activity_events", idOf(event)), source: event.source_type, metric: event.metric_key, quantity: event.quantity || 1, occurred_at: event.occurred_at, metadata: event.metadata || {} })) },
       ...
       deep_work: deepWork.slice(0, 12).map((item) => ({ evidence_id: evidenceId("deep_work_logs", idOf(item)), date: item.logged_on || item.created_at, area: item.area, focus: item.focus, minutes: item.duration_minutes, output: item.output })),
       ...
       content_library: contentItems.slice(0, 12).map((item) => ({ evidence_id: evidenceId("content_items", idOf(item)), title: item.title, type: item.content_type || item.type, status: item.status, logged_on: item.logged_on || item.created_at })),
      ...
      capability_system: { practical: capabilities.filter((item) => item.skill_type === "Practical").map((item) => ({ id: item.id, title: item.title, status: item.status, practice_count: Number(item.practice_count || 0), last_practiced_on: item.last_practiced_on || null, latest_note: item.latest_note || null })), adversarial: capabilities.filter((item) => item.skill_type === "Adversarial").map((item) => ({ id: item.id, title: item.title, status: item.status, practice_count: Number(item.practice_count || 0), last_practiced_on: item.last_practiced_on || null, latest_note: item.latest_note || null })), recent_logs: capabilityLogs.slice(0, 24).map((item) => ({ evidence_id: evidenceId("capability_skill_logs", idOf(item)), skill_id: item.skill_id, practiced_on: item.practiced_on, duration_minutes: item.duration_minutes, pressure_level: item.pressure_level, result: item.result })) },
      business_financial_foundation: financialFoundation ? { monthly_income: numberValue(financialFoundation.monthly_income), monthly_expenses: numberValue(financialFoundation.monthly_expenses), liquid_reserves: numberValue(financialFoundation.liquid_reserves), emergency_fund_target: numberValue(financialFoundation.emergency_fund_target), debt_balance: numberValue(financialFoundation.debt_balance), business_revenue: numberValue(financialFoundation.business_revenue), runway_months: numberValue(financialFoundation.monthly_expenses) > 0 ? Number((numberValue(financialFoundation.liquid_reserves) / numberValue(financialFoundation.monthly_expenses)).toFixed(2)) : null, notes: financialFoundation.notes || null } : null,
      special_projects: { projects: projects.slice(0, 8).map((project) => ({ id: project.id, parent_project_id: project.parent_project_id || null, title: project.title, project_type: project.project_type || "Real-world project", project_mode: project.project_mode || "Milestone", effort_band: project.effort_band || "Standard", estimated_hours: project.estimated_hours || null, completion_xp: project.xp_reward ?? 60, status: project.status, priority: project.priority, progress: Number(project.progress || 0), outcome: project.outcome || null, completion_evidence: project.completion_evidence || null, next_action: project.next_action || null, due_on: project.due_on || null, logged_on: project.logged_on || project.created_at })) },
      ...
      account_deposit_ledger: accountDeposits.slice(0, 40).map((item) => ({ account_id: item.account_id, amount_usd: item.amount_usd, deposited_at: item.deposited_at, note: item.note || null })),
