...
const debriefRules = `For bedtime and evening debriefs, use at least two distinct references: one concrete operating-day record and one comparison or historical reference from derived_insights, mission outcomes, feedback, scenario results, or advisory_history. Do not repeat the previous debrief's wording; if a pattern is unchanged, name the new or repeated evidence and its operational implication. tomorrow_focus is a read-only pointer, never a new assignment: operation_title must exactly copy the title of one unfinished, concrete operation already present in the supplied operations context and suitable for the next operating day. Never use a mission title, phase title, combined mission phrase, or a vague verb such as "Advance." If no supplied operation is a defensible match, use "No existing operation selected".`;
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const modeRules = req.body?.mode === "bedtime"
      ? "This is the user's Going to bed evening debrief. Review the operating day represented by evidence_date, including late activity before sleep. The debrief itself is read-only: do not issue directives, missions, roadmap changes, or direct queue edits. Tomorrow's Focus may only point to an existing concrete operation from the supplied context; it must never create or imply a new operation."
