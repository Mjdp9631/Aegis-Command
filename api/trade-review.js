...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const pre = await responsesRequest(process.env.OPENAI_API_KEY, preSystem, [{type:"input_text",text:"LOGGED METADATA (outcome and post-entry fields removed):\\n" + JSON.stringify(metadata)}, ...labeledFrames(screenshots)], "clarified_chaos_blind_scenario", preTradeSchema);
    ...
    const review = await responsesRequest(process.env.OPENAI_API_KEY, finalSystem, [{type:"input_text",text:context}, ...labeledFrames(screenshots)], "clarified_chaos_trade_review", finalSchema);
