...
function characterFocus(metrics, recovery) {
  ...
  const axisMarkup = radarData.map((axis) => {
    ...
    const node = `M ${x} ${Number(axis.pointY - 2.4).toFixed(2)} L ${Number(axis.pointX + 2.1).toFixed(2)} ${Number(axis.pointY - 1.2).toFixed(2)} L ${Number(axis.pointX + 2.1).toFixed(2)} ${Number(axis.pointY + 1.2).toFixed(2)} L ${x} ${Number(axis.pointY + 2.4).toFixed(2)} L ${Number(axis.pointX - 2.1).toFixed(2)} ${Number(axis.pointY + 1.2).toFixed(2)} L ${Number(axis.pointX - 2.1).toFixed(2)} ${Number(axis.pointY - 1.2).toFixed(2)} Z`;
    return `<g class="character-radar-level" data-focus-axis="${axis.label.toLowerCase()}" tabindex="0" role="button" aria-label="${axis.label} level"><line class="character-radar-spoke" x1="50" y1="50" x2="${axis.x}" y2="${axis.y}"></line><path class="character-radar-node" d="${node}"></path><circle class="character-radar-hit" cx="${x}" cy="${y}" r="7"></circle><text class="character-radar-label" x="${axis.labelX}" y="${axis.labelY}" text-anchor="${axis.anchor}">${axis.label}</text></g>`;
  }).join("");
  return `<section class="character-focus panel"><div class="character-focus-heading"><p class="eyebrow blue-text">CHARACTER SYSTEMS / LIVE PROFILE</p><h3>The whole system at a glance.</h3><p>Hover or focus an axis to isolate that level. Each line reflects evidence earned in its system.</p></div><div class="character-focus-layout"><div class="character-hexagon-wrap"><div class="character-hexagon"><svg viewBox="0 0 100 100" role="img" aria-label="Character level radar"><polygon class="character-radar-frame" points="50,4 91,27 91,73 50,96 9,73 9,27"></polygon><polygon class="character-radar-grid" points="50,7 87,28 87,72 50,93 13,72 13,28"></polygon><polygon class="character-radar-grid inner" points="50,22 74,36 74,64 50,78 26,64 26,36"></polygon><polygon class="character-radar-frame inner-frame" points="50,14 81,32 81,68 50,86 19,68 19,32"></polygon><path class="character-radar-axis" d="M16 50H84M50 7V93M16 28L84 72M84 28L16 72"></path><circle class="character-radar-ring" cx="50" cy="50" r="38"></circle><path class="character-radar-scan" d="M50 50L50 7"></path><polygon class="character-radar-data" points="${points}"></polygon>${axisMarkup}<circle class="character-radar-core-ring" cx="50" cy="50" r="5.5"></circle><circle class="character-radar-core" cx="50" cy="50" r="1.8"></circle></svg></div><div class="character-hexagon-readout"><span>CHARACTER LEVEL</span><strong>LV ${level.level}</strong><small>${Math.round(level.current)} / ${level.required} XP</small></div></div><div class="character-focus-stats">${focusStat("DISCIPLINE", metrics.discipline, "daily", "amber", "discipline")}${focusStat("TRADING INTEL", metrics.trading, "monthly", "blue", "trading")}${focusStat("BODY MASTERY", metrics.mastery.body, "Body", "green", "body")}${focusStat("CCFX QUESTS", metrics.ccfx, "CCFX", "amber", "ccfx")}${focusStat("MIND MASTERY", metrics.mastery.mind, "Mind", "blue", "mind")}${recoveryFocusStat(recovery)}</div></div></section>`;
}
