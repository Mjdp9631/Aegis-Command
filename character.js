...
function masteryXp(entries, startedAt) {
...
function render({ operations, trades, missions, projects, contentItems, masteryEntries }) {
  ...
  const mastery = startedAt ? masteryXp(masteryEntries, startedAt) : { mind: empty, body: empty };
...
async function load() {
  ...
  const [operationsResult, tradesResult, missionsResult, projectsResult, contentResult, masteryResult, campaignResult] = await Promise.all([
