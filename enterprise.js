...
async function createProjectStepOperation(project, step) {
  ...
  const { data, error } = await supabase.from("operations").insert({
    user_id: userId,
    title: projectStepOperationTitle(project, step),
    category: "Business",
    priority: projectOperationPriority(project.priority),
    status: "Queued",
    completed: false,
    ...
    is_daily: false,
    allow_unlinked: true,
    brief: `Project step ${step.position}: ${step.title}`,
  }).select().single();
...
async function synchronizeProjectStep(operation) {
  ...
  const status = operation.completed || String(operation.status || "").toLowerCase() === "complete"
    ? "Complete"
    : String(operation.status || "").toLowerCase() === "ongoing" ? "Ongoing" : "Pending";
...
function buildDialogs() {
  ...
  $("#project-dialog form")?.addEventListener("submit", async (event) => {
    ...
    const { data: project, error } = await supabase.from("business_projects").insert({ logged_on: $("#project-logged-on").value || easternDateKey(), title, project_type: $("#project-type").value, project_mode: projectMode, effort_band: effortBand, estimated_hours: Number($("#project-estimated-hours").value) || null, xp_reward: projectMode === "Ongoing system" ? 0 : PROJECT_XP[effortBand], status: "Active", priority: $("#project-priority").value, progress: 0, outcome: $("#project-outcome").value.trim(), next_action: steps[0], due_on: $("#project-due").value || null }).select().single();
