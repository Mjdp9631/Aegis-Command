...
async function load() {
  ...
  const [projectResult, contentResult] = await Promise.all([supabase.from("business_projects").select("*").order("created_at", { ascending: false }), supabase.from("content_items").select("*").order("created_at", { ascending: false })]);
...
function buildDialogs() {
  ...
  $("#project-dialog form").addEventListener("submit", async (event) => { event.preventDefault(); const title = $("#project-title").value.trim(); if (!title) return; const { error } = await supabase.from("business_projects").insert({ title, status: $("#project-status").value, priority: $("#project-priority").value }); if (error) return alert(error.message); $("#project-dialog").close(); $("#project-title").value = ""; load(); });
  $("#content-dialog form").addEventListener("submit", async (event) => { event.preventDefault(); const title = $("#content-title").value.trim(); if (!title) return; const { error } = await supabase.from("content_items").insert({ title, platform: $("#content-platform").value, status: $("#content-status").value }); if (error) return alert(error.message); $("#content-dialog").close(); $("#content-title").value = ""; load(); });
}
