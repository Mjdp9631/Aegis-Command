...
function render({ operations, missions, trades }) {
  ...
  const closedTrades = trades.filter((trade) => String(trade.account || "").trim().toLowerCase() !== "theoretical" && outcomeOf(trade) !== "Open");
  ...
  panel.innerHTML = `<div class="adviser-head"><p class="eyebrow blue-text">EVENING DEBRIEF</p><h3>EXAMINATION / REFLECTING</h3></div><div class="evening-columns"><article><span>KEY TAKEAWAYS</span><p><b>JARVIS</b>${completed}/${operations.length || 0} operations complete. Closed-trade win rate: ${winRate}.</p><p><b>ALFRED</b>${completed ? "Execution is on record. Preserve the standard tomorrow." : "No execution has been logged yet; an honest reset is still available."}</p></article><article><span>WHAT WORKED</span><p><b>JARVIS</b>${completed ? `${completed} commitment${completed === 1 ? " was" : "s were"} completed.` : "No completed operations to measure yet."}</p><p><b>ALFRED</b>${completed ? "You kept at least part of the agreement with yourself." : "Begin with the smallest clear commitment, sir."}</p></article><article><span>WHAT TO IMPROVE</span><p><b>JARVIS</b>${remaining.length ? `${remaining.length} open commitment${remaining.length === 1 ? " remains" : "s remain"}.` : "No outstanding commitments remain."}</p><p><b>ALFRED</b>${remaining.length ? "Do not carry indecision into tomorrow; choose the first action now." : "Close the day properly and allow recovery to do its work."}</p></article><article><span>TOMORROW'S FOCUS</span><p><b>JARVIS</b>${nextOperation}</p><p><b>ALFRED</b>Protect the plan by making this the first decision of the day.</p></article></div>`;
}
...
if (supabase) {
  loadAdvisory();
  supabase.auth.onAuthStateChange((event) => { if (event !== "SIGNED_IN") return; setTimeout(loadAdvisory, 100); });
